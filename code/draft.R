...
# Create a table with carrier name and plane manufactured year merged by tail number
# carrier_tailnum <- merge(fd_comp, carriers, by.x = "UniqueCarrier", 
#                          by.y = "Code", all.x = TRUE, all.y = FALSE) %>% 
#   select(Carrier = Description, TailNum) %>% 
#   distinct()

# plane_reldate <- merge(carrier_tailnum, plane_reldate, by = "TailNum") %>% 
#   filter(Release <= 2000)


fd_new <- fd_comp %>% 
  mutate(DepDel0 = ifelse(DepDelay < 15, 0, DepDelay),
         DepDel15 = ifelse(DepDelay >= 15, 1, 0),
         ArrDel0 = ifelse(ArrDelay < 15, 0, ArrDelay),
         ArrDel15 = ifelse(ArrDelay >= 15, 1, 0))

fd_reldate <- merge(fd_new, plane_reldate, by = "TailNum") %>%
  mutate(Delayed = ifelse(DepDel15 == 1 | ArrDel15 == 1, 1, 0)) %>% 
  select(TailNum, UniqueCarrier, Delayed, Release) %>% 
  group_by(TailNum, UniqueCarrier, Release) %>% 
  summarise(Delayed_Count = sum(Delayed))
  

plcr_reldate <- merge(fd_reldate, carriers, by.x = "UniqueCarrier", by.y = "Code",
                       all.x = TRUE, all.y = FALSE)
  

# plcr_reldate %>% count(length(unique(TailNum)))

# From plane data, we calculate the average manufacturing year as a reference, where
...
plane_data %>%
  ...
  select(year) %>%
  arrange(year) %>% 
  summarise(AvgYear = round(mean(year), 0))
...
# Conduct a hypothesis test to compare equality of the two variance
res.ftest <- var.test(Count ~ Age, data = test)
...
# Null hypothesis: average new planes delay - average old plane delay >= 0
# Alt hypothesis: average new planes delay - average old plane delay < 0
res <- t.test(Count ~ Age, data = test, var.equal = FALSE, alternative = "less")
